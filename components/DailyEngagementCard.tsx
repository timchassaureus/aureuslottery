...
export default function DailyEngagementCard({
  ...
  return (
    <div className="rounded-2xl border border-[#C9A84C]/ bg-black/40 p-5">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <Sparkles className="h-5 w-5 text-yellow-300" />
          <h3 className="text-base font-bold text-white">Daily Missions</h3>
        </div>
        <div className="inline-flex items-center gap-1 rounded-full border border-[#C9A84C]/ bg-[#C9A84C]/ px-2.5 py-1 text-xs text-[#F5F0E8]">
      ...
      <div className="mt-4 space-y-2.5">
        ...
        <div className="flex items-center justify-between rounded-lg border border-white/10 bg-black/20 px-3 py-2">
          ...
          {state.completed.buy ? (
            ...
            <span className="text-xs text-[#e8c97a]">Pending</span>
          )}
        </div>

        <button
          onClick={markShareDone}
          className="flex w-full items-center justify-between rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-left hover:border-[#C9A84C]/ transition-colors"
        >
          <span className="text-sm text-white/90">Invite 1 friend / share link</span>
          {state.completed.share ? (
            <CheckCircle2 className="h-4.5 w-4.5 text-emerald-400" />
          ) : (
            <span className="text-xs text-[#F5F0E8]">Tap to do</span>
          )}
        </button>
      </div>
      ...
      <div className="mt-4">
        <div className="mb-1.5 flex items-center justify-between text-xs">
          <span className="text-[#F5F0E8]/">Daily progress</span>
      ...
      <div className="mt-4 rounded-lg border border-white/10 bg-black/20 p-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-[#F5F0E8]/">{lifetimeTickets} lifetime tickets</span>
        </div>
        {nextTier ? (
          ...
          <p className="mt-1 text-xs text-[#F5F0E8]">Max tier unlocked.</p>
        )}
      </div>

      <div className="mt-3 rounded-lg border border-white/10 bg-black/20 p-3">
        <div className="mb-1.5 flex items-center justify-between text-xs">
          <span className="inline-flex items-center gap-1 text-[#F5F0E8]/">
        ...
        <span
          className={`inline-flex items-center gap-1 rounded-lg border px-3 py-1.5 text-xs ${
            completedAll
              ...
              : 'border-[#C9A84C]/ bg-[#C9A84C]/ text-[#F5F0E8]'
          }`}
        >
      ...
      <p className="mt-2 text-[11px] text-[#F5F0E8]/">
        XP: <span className="font-semibold text-white">{xpPoints}</span> •
        {' '}Comeback bonus multiplier: <span className="font-semibold text-[#F5F0E8]">x{Math.min(5, Math.max(1, state.streakDays))}</span>
      </p>
    </div>
  );
}
