...
export default function ReferralDashboard() {
  ...
  if (!wallet) {
    return (
      <div className="rounded-xl border border-[#C9A84C]/ bg-black/40 p-6 text-center">
        <p className="text-[#e8c97a]/">Connect your wallet to get your referral link and earn 3% on every ticket your friends buy.</p>
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center gap-2 rounded-xl border border-[#C9A84C]/ bg-black/40 p-8">
        <Loader2 className="h-6 w-6 animate-spin text-[#C9A84C]" />
        <span className="text-[#e8c97a]/">Loading referral data…</span>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-[#C9A84C]/ bg-black/40 p-6 space-y-6">
      ...
      {/* How it works */}
      <div className="rounded-xl border border-[#C9A84C]/ bg-[#C9A84C]/ p-4 space-y-3">
        <p className="text-sm font-semibold text-white">How it works</p>
        <ol className="space-y-2 text-sm text-[#F5F0E8]/">
        ...
        <p className="text-xs text-[#C9A84C]/ border-t border-[#C9A84C]/ pt-2">
          No cap, no expiry. The more friends you bring, the more you earn — even while you sleep.
        </p>
      </div>
      {referredCount >= 10 && (
        <div className="rounded-lg bg-[#C9A84C]/ border border-[#C9A84C]/ px-3 py-2 flex items-center gap-2 text-[#e8c97a] font-medium">
          🏆 Super Referrer
        </div>
      )}
        ...
        <>
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              readOnly
              value={referralLink}
              className="flex-1 rounded-lg border border-[#C9A84C]/ bg-black/40 px-4 py-3 text-sm text-white font-mono"
            />
            <button
              onClick={handleCopy}
      ...
      <div className="grid grid-cols-2 gap-4 pt-4 border-t border-[#C9A84C]/">
        <div className="flex items-center gap-3">
          <div className="rounded-lg bg-black/40 p-3 border border-[#C9A84C]/">
            <Users className="w-6 h-6 text-primary-400" />
          </div>
          <div>
            <p className="text-2xl font-bold text-white">{referredCount}</p>
            <p className="text-xs text-[#C9A84C]/">Friends referred</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="rounded-lg bg-black/40 p-3 border border-[#C9A84C]/">
          ...
          <div>
            ...
            <p className="text-xs text-[#C9A84C]/">Total earned</p>
          </div>
        </div>
      </div>
    </div>
